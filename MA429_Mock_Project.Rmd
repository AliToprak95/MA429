...
Native.countryplot
library(ggpubr)
figure <- ggarrange(Native.countryplot,Capital.lossplot,
                    labels = c("Native Country", "Capital Loss"),
                    nrow = 2)
figure
```
...
a<-rbind(
  data.frame(bins$Age),
  data.frame(bins$Workclass),
  data.frame(bins$Fnlwgt),
  data.frame(bins$Education),
  data.frame(bins$Education.num),
  data.frame(bins$Marital.status),
  data.frame(bins$Occupation),
  data.frame(bins$Relationship),
  data.frame(bins$Race),
  data.frame(bins$Sex),
  data.frame(bins$Capital.gain),
  data.frame(bins$Capital.loss),
  data.frame(bins$Hours.per.week),
  data.frame(bins$Native.country)
)
library(xlsx)
write.xlsx(a, "C:/Users/Ali/Desktop/MA429.xlsx") 
bins
```
...
write.xlsx(cordata, "C:/Users/Ali/Desktop/MA429.xlsx") 
correlations
...
library(ROCR)
colnames(train_woe)
lr <- glm(Income.level ~ . -Native.country_woe , data = train_woe[,-13], family = binomial())
...
auc <- performance(xgb_prediction,measure = "auc")
lift <- performance(xgb_prediction, measure="lift", x.measure="rpp")
...
list("decision tree" = getMetrics(test_woe$Income.level, tree_predict))
```
#ROC CURVE PLOTTING
```{r}
